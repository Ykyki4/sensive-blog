...
def index(request):
    ...
    fresh_posts = Post.objects.order_by('published_at').prefetch_related('author').annotate(comments_count=Count('comments'))
    most_fresh_posts = list(fresh_posts)[-5:]

    tags = Tag.objects.annotate(related_posts=Count('posts', distinct=True))\
        .order_by('-related_posts')
    most_popular_tags = tags[0:5]
